...
public class Chat : NetworkBehaviour {
    ...
    public ChannelInfo team = new ChannelInfo("", "(Team)", "(Team)", Color.cyan);
    public ChannelInfo all = new ChannelInfo("/all", "(All)", "(All)", Color.white);
    ...
    void CmdMsgTeam(string message) {
        ...
        foreach(var entry in  NetworkIdentity.spawned) {
            ...
            if (chat != null &&
                ...
                chat.TargetMsgTeam(name, message);
            }
        }
    }

    [Command] // unimportant => unreliable
    void CmdMsgWhisper(string playerName, string message) {
        ...
        foreach(var entry in  NetworkIdentity.spawned) {
            ...
            if (entry.Value.name == playerName && chat != null) {
                ...
                TargetMsgWhisperTo(entry.Value.name, message);
                return;
            }
        }
    }
    ...
    [TargetRpc] // only send to one client
    public void TargetMsgAll(string sender, string message) {
        AddMessage(new MessageInfo(sender, all.identifierIn, message, "", all.color));
    }

    [TargetRpc] // only send to one client
    public void TargetMsgTeam(string sender, string message) {
